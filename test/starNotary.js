...
it('token name and token symbol are added', async() => {
    
    assert.equal(await instance.getName(), 'SaraToken');
    assert.equal(await instance.getSymbol(), 'SAT');
  });
...
it('2 users can exchange their stars', async() => {
    ...
    await instance.createStar('awesome star', starId, {from: userFrom})
